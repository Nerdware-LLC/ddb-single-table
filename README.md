...
- _Max nest depth for auto-generated typings of nested `schema` attributes:_ <br>
  Currently, for nested `schema` attributes the `ItemTypeFromSchema` utility-type only permits a maximum nesting depth of 5 for performance reasons. This is due to the fact that the typings are generated by recursively traversing the schema object, and the deeper the nesting the more recursive calls are required. This is a temporary limitation, and is targeted for remediation in a future release.
